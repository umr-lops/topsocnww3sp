...
def read_osw(group,lst_sar_files_osw,dev=False):
    ...
    for ii in tqdm(range(len(lst_sar_files_osw))):
        ...
        coords_osw['lon_osw'] = np.hstack([coords_osw['lon_osw'],dsosw['oswLon'].squeeze().values.ravel()])
        coords_osw['lat_osw'] = np.hstack([coords_osw['lat_osw'],dsosw['oswLat'].squeeze().values.ravel()])
        # dsosw = dsosw.isel({'oswKyBinSize':slice(0,90)})

        if dsosw['oswKyBinSize'].size < min_kybinsize:
            min_kybinsize = dsosw['oswKyBinSize'].size
        ...
        if (dsosw['oswLandFlag'].values==0).any(): #it means at least one tile is over ocean
    ...
    logger.info('Number of subswath concatenated: %d, discarded because only land: %d',cpt_subswath_concat,cpt_subswath_discard)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Read S1 OSW tops data and concatenate along tiles dimension')
    parser.add_argument('--group', type=str, default='intraburst', help='Group in the osw file to read, can be intraburst or interburst')
    parser.add_argument('--path_to_sar_files', type=str, required=True, 
                help='Path to the directory containing the SAR files with osw data, e.g. /path/to/S1/files/*SAFE/measurement/*osw*nc')
    parser.add_argument('--logging_level', type=str, default='INFO', help='Logging level, e.g. INFO, DEBUG')
    ...
    lst_sar_files_osw = glob.glob(os.path.join(args.path_to_sar_files,'*SAFE','measurement','*osw*nc'))
    logger.info('Found %d osw files '%(len(lst_sar_files_osw)))
    fat_osw,coords_osw = read_osw(args.group,lst_sar_files_osw,dev=args.dev)
    logger.info('Done reading osw files, fat_osw shape: %s',str(coords_osw['lon_osw'].shape))
