...
def collect_each_matching_locations(safedir,dirout):
    ...
    for ii in pbar:
        try:
            ...
            date = datetime.datetime.strptime(os.path.basename(lst_osw[ii]).split('-')[5],'%Y%m%dt%H%M%S')
    ...
    if len(lons_match)>0:
        logging.info('lon/lat range: %1.2f/%1.2f to %1.2f/%1.2f',np.min(lons_match),np.min(lats_match),np.max(lons_match),np.max(lats_match))
        df['lon'] = lons_match
        df['lat'] = lats_match
    ...
    if len(dates)>0:
        fout = os.path.join(dirout,'trackfile-ww3spectra-%s.txt'%(base_safe.replace('.SAFE','')))
...
def entry_point_ocn_between_dates():
    ...
    parser.add_argument('--verbose', action='store_true', default=False)
    parser.add_argument('--outputdir', required=True, help='directory where to store output')
    parser.add_argument('--inputdir', required=True, help='directory where to OCN are stored. up to e.g. .../S1A_IW_OCN__2S/ ')
