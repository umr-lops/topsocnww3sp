...
from topsocnww3sp.l2c_processor import haversine, find_ww3_file, process_group
...
def dummy_ww3_ds():
    ...
    ds = xr.Dataset(
        data_vars={
            "efth": (["time", "frequency", "direction"], np.random.rand(5, 3, 4).astype('f4')),
...
def dummy_sar_ds():
    ...
    ds = xr.Dataset(
        data_vars={
            "oswLon": (["subswath", "oswRaSize", "oswAzSize"], np.full((1, 2, 3), -5.0)),
            "oswLat": (["subswath", "oswRaSize", "oswAzSize"], np.full((1, 2, 3), 48.01)),
...
def test_process_group_modes(mode, dummy_sar_ds, dummy_ww3_ds, mock_config):
    ...
    with patch("topsocnww3sp.l2c_processor.read_osw", return_value=(dummy_sar_ds, None)):
...
def test_main_integration(monkeypatch, mock_config, dummy_ww3_ds):
    ...
    mock_args = MagicMock()
    mock_args.osw_file = "s1a-iw1-osw-vv-20220107t062429-20220107t062500-041351-04ea80-001.nc"
