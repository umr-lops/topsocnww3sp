...
from topsocnww3sp.count_ocn_tiles_with_ww3sp import core_count_coverage, parse_track_file
    ...
    for path in input_paths:
        if path.endswith('.txt'):
            ...
            with open(path, 'r') as f:
                files_from_txt = [line.strip() for line in f if line.strip() and not line.startswith('#')]
...
def main():
    parser = argparse.ArgumentParser(description="Map OSW tiles, Trackfile, and WW3 spectra with stats.")
    ...
    summary_lines, results = core_count_coverage(track_points, ww3_lons, ww3_lats,
                                                 ww3_times, pathconfig=args.config)
    summary_text = "\n".join(summary_lines)
    ...
    extent = [np.min(all_lons)-0.3, np.max(all_lons)+0.3, np.min(all_lats)-0.3, np.max(all_lats)+0.3]
    ax1.set_extent(extent, crs=ccrs.PlateCarree())
    ax1.add_image(request, args.zoom)

    ax1.scatter(coords_osw['lon_osw'], coords_osw['lat_osw'], transform=ccrs.PlateCarree(), 
                s=16, c='magenta', marker='s', edgecolors='white', label='S1 OSW (%i)' % len(coords_osw['lon_osw']))
    ax1.scatter(t_lons, t_lats, transform=ccrs.PlateCarree(), 
                s=14, c='yellow', marker='D', edgecolors='black', label='Trackfile (%i)' % len(t_lons), alpha=0.6)
    ax1.scatter(ww3_lons, ww3_lats, transform=ccrs.PlateCarree(), s=8, c='cyan', alpha=0.3, label='WW3 (%i)' % len(ww3_lons))

    ax1.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    ax1.set_title(f"S1 OCN/WW3 Overview - {args.group}", fontsize=14)

    # Summary box on the right
    fig1.text(0.72, 0.5, summary_text, fontsize=8, family='monospace', 
              verticalalignment='center', bbox=dict(boxstyle='round', facecolor='white', alpha=0.9))
    ...
    ax2.scatter(t_lons[mask_zero], t_lats[mask_zero], transform=ccrs.PlateCarree(),
                s=30, c='red', marker='o', edgecolors='black', label='0 WW3 Spectra')

    # 2. Plot >0 hits with graduate colors (using 'viridis' or 'plasma' for better visibility on satellite)
    if np.any(mask_pos):
        sc = ax2.scatter(t_lons[mask_pos], t_lats[mask_pos], transform=ccrs.PlateCarree(),
                         s=30, c=t_hits[mask_pos], cmap='viridis', edgecolors='white', 
                         linewidth=0.5, label='>0 WW3 Spectra')
    ...
    ax2.set_title(f"Track Points: Hit Count Distribution\n(Red = No spectra within {config['DISTANCE_THRESHOLD_KM']}km/{config['TIME_THRESHOLD_MINUTES']}min)", fontsize=14)
