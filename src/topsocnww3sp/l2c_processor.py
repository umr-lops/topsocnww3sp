...
def process_group(osw_path, ds_ww3, group_name, config, sar_start, mode):
    ...
    sar_flat = fat_osw.reset_index('tiles').stack(all_tiles=['subswath', 'tiles']).dropna('all_tiles', subset=['oswLon'])
    ...
    if mode == '1to1':
        ...
        ds_match = xr.Dataset({
            "distance_km": (["all_tiles"], pd.Series(distances, index=sar_indices).reindex(tile_coords).values),
        ...
        ds_ww3_out = ds_ww3.isel(time=cand_idx_orig[unique_rel_idx]).rename({'time': 'unique_ww3'})
...
def main():
    ...
    parser.add_argument("--group", choices=["intraburst", "interburst", "both"], default="both")
    ...
    ww3_path = args.ww3_file if args.ww3_file else find_ww3_file(sar_start, config)
